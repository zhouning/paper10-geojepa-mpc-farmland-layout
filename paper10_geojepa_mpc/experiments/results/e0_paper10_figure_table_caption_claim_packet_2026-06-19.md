...
| Main Table 2 | main | preview_available | Journal-neutral draft caption: frozen matched-baseline table reports the positive Bishan anchor and Stage 3 boundary rows using tracked Stage 3 and raw-rollout consistency evidence. | Table 1 is the only positive Bishan performance anchor<br>Stage 3 rows are boundary evidence<br>diagnostic near-pass must not be pooled | Do not rewrite Stage 3 boundary rows as direct 50-state Bishan scale-up success.<br>Do not add unsupported comparison-testing wording to the frozen table. | rounding<br>main-text placement<br>target-journal caption length |
