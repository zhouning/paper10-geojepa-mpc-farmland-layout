...
6. Confirm public-facing root docs and integrated manuscript drafts use specific
   data/access-route wording, not "available upon request" or temporary cloud
   link language.
7. Confirm every citation key used by the public manuscript, citation map, and
   citation checklist exists in the verified or local BibTeX files.
8. Confirm the public self-contained manuscript body does not cite
   `@zhou2026paper9_local`.
9. Grep for prohibited passing-50-state wording and keep only guardrail or
   failed-diagnostic uses.
10. Run the full Paper10 test suite:

```powershell
D:\adk\.venv\Scripts\python.exe -m pytest paper10_geojepa_mpc\tests -q -p no:cacheprovider
```

11. If possible, clone the archive candidate into a clean directory and rerun
...
12. Record the final submission commit, test result, DOI or reviewer link, and
