...
Status: clean CEUS main-manuscript draft, updated by the 2026-07-09 CEUS policy verification, guard information-set audit and proxy guard dynamic baseline stress audit for bounded formal submission. This file removes author handoff notes, source-control lists, terminology ledgers, claim-lock tables and unresolved-blocker tables from the manuscript body. It adds a 5-seed no-oracle proxy guard stress-test result and keeps the bounded claim boundary while treating remaining author names, affiliations, CRediT roles, declarations, funding and upload steps as submission-system fields rather than algorithm, experiment, archive or artwork blockers.
...
- Oracle action-audit guard bounds the Bishan primary reward evidence.
- No-oracle proxy guards failed to beat the 5-seed value-filter anchor.
...
The evidence ladder is deliberately bounded. Bishan now provides positive reward evidence through the CEUS review-response experiment package: a 20-seed rewardtop7 true-reward margin guard improved reward over the value-filter baseline in the 20x16/top5 setting while keeping secondary planning metrics mixed. The guard information-set audit bounds this as oracle/action-audit evidence because it uses privileged immediate true reward over the audited action set. A new 5-seed no-oracle proxy guard stress test did not beat the value-filter anchor, so proxy diagnostics remain boundary evidence rather than a deployable-policy claim. Stage 3 then tested whether authorized 50-state value-label rows could support a stronger scale claim. They completed value-filter rollouts but remained below the matched comparator. Dongxing/Neijiang provides second-region calibration and stress-test evidence, but mixed transfer-versus-scratch outcomes prevent a transfer-superiority claim.
...
### 3.2 Bishan 20x16/top5 oracle action-audit guard bounded the primary reward evidence

The CEUS review-response experiment package `e0_paper10_ceus_review_response_experiment_package_2026-07-09` makes the 20-seed rewardtop7 true-reward margin guard the current oracle/action-audit Bishan reward evidence. Under the Bishan 20x16/top5 protocol, the true-reward margin guard reached 72.1918 mean reward compared with 65.8876 for the value-filter baseline. The paired mean delta was 6.3041 reward units, with bootstrap 95% CI 4.1401 to 8.5056. The guard won 20/20 paired seeds, and the minimum seed-level delta remained positive at 0.0029.

The guard is an executable action-audit procedure rather than a wording change, but its information set is privileged: after value-filter selection, it audits the selected action plus the model-reward top7 actions with immediate true reward and switches only when the audited best true reward exceeds the selected action by the setting-specific margin 1.50. Across 2000 audited states it switched 172 times, for a switch rate of 0.0860. The simplified rewardtop7 audit used 7.7605 mean audited actions, lower than the dual7x7 diagnostic path at 8.1905 mean audited actions. The guard information-set audit `e0_paper10_guard_information_set_audit_2026-07-09` therefore labels it as an oracle/action-audit guard and not a standalone deployable no-oracle planner.
...
A new no-oracle proxy guard stress test, `e0_paper10_proxy_guard_dynamic_baseline_audit_2026-07-09`, tested whether score-space margin guards could replace the true-reward audit in dynamic rollout. `model_reward_proxy_guard_m010` reached 65.2734 mean reward across seeds 0-4, and `candidate_score_proxy_guard_m010` reached 63.4116. Both were below the 69.4705 value-filter anchor despite switch rates of 0.1980 and 0.1960, respectively. Statewise proxy gains therefore did not translate into dynamic rollout superiority. Proxy-guard design remains an experiment boundary rather than the primary claim.
...
The review-response experiment package narrows the paper while strengthening its bounded reward evidence. The supported contribution is not broad value-filter superiority across seeds, regions or label scales. The supported contribution is an evidence-controlled workflow that records when a value-label setting is usable, when executable constraints are essential, and when an oracle/action-audit true-reward margin guard improves the reward outcome under a bounded protocol.

The Bishan 20x16/top5 result is now centered on the 20-seed rewardtop7 oracle/action-audit guard, which reached 72.1918 mean reward versus 65.8876 for the value-filter baseline and won all 20 paired seeds. This is not evidence of a deployable no-oracle guard. The new 5-seed proxy stress audit found that model-reward and candidate-score proxy guards remained below the value-filter anchor, so the no-oracle guard pathway is still unresolved. The old 5-seed value-filter anchor remains useful because it explains the predecessor policy and matched comparator context, but its 3/5 seed-wise outcome and diagnostic-only sign-test readout of 1.0000 prevent it from carrying the primary claim.
...
Several limitations remain for interpretation. First, the current evidence is two-region and descriptive. Broader generalization claims require additional external regions and a predefined comparison protocol. Second, the pairwise-only baseline policy remains unresolved unless the author team explicitly accepts the matched `rank_seed2028` comparator as the route comparator. Third, no-oracle proxy guards were tested only as 5-seed stress baselines and did not beat the value-filter anchor; a deployable guard would require a new proxy design and a predefined 20-seed rollout protocol. Fourth, original Bishan and Dongxing DLTB inputs are confidential and cannot be externally provided, so full raw-DLTB reruns remain limited to the author-controlled environment. The 2026-07-09 CEUS policy verification treats this as a required disclosure in the Data and Code Availability statement, not as a pre-submission algorithm or experiment blocker.

## 5. Conclusion

Paper10 supports monitor-gated value labels, executable masks and a setting-specific oracle/action-audit guard as a bounded GeoJEPA-MPC workflow for constrained farmland layout planning. The primary Bishan 20x16/top5 reward evidence is the 20-seed rewardtop7 guard, which improves mean reward over the value-filter baseline with a positive bootstrap interval while leaving secondary planning indicators mixed. It is not a deployable no-oracle planner: 5-seed model-reward and candidate-score proxy guard stress tests remained below the value-filter anchor. The legacy 5-seed value-filter anchor, Stage 3 boundary rows, candidate-score sweep and Dongxing/Neijiang stress tests prevent stronger claims about universal margins, 50-state scale-up, transfer superiority or operational cadastral deployment. These boundaries mean the result is not broad scale-up, transfer superiority or operational cadastral deployment. The CEUS manuscript should therefore present the work as a reproducible evidence-control and planning-support workflow with explicit boundaries.

## Data and Code Availability

This CEUS-facing statement is updated by `e0_paper10_ceus_submission_policy_verification_2026-07-09.md`, and the algorithm/experiment hierarchy is locked by `e0_paper10_ceus_review_response_experiment_package_2026-07-09.md`, `e0_paper10_guard_information_set_audit_2026-07-09.md` and `e0_paper10_proxy_guard_dynamic_baseline_audit_2026-07-09.md`. The reviewer-facing repository route is the author-confirmed 4open README.md direct link: `https://anonymous.4open.science/r/geojepa-mpc-farmland-layout-8552/README.md`. The current GitHub submission-preparation commit anchor before this proxy-stress manuscript refresh is `b5457460e747cc320e2246dbfcd30e082851c01a`. The author checked the 4open page on 2026-07-09 and reported no visible exact snapshot identifier, version identifier or commit hash; this GitHub commit is therefore a submission-preparation anchor, not proof of the exact 4open snapshot. Under the checked CEUS/Elsevier Research Data Policy Option B route, the confidential raw-DLTB limitation is disclosed in this section and does not require pre-submission editor acceptance before the manuscript can be submitted for review.
...
Figure 2. Bishan 20x16/top5 oracle action-audit guard result. The figure should report the 20-seed rewardtop7 guard and value-filter baseline rewards, the 6.3041 paired mean delta, bootstrap 95% CI 4.1401 to 8.5056, 20/20 seed wins and the mixed secondary-metric boundary.
...
Table 2. Bishan 20-seed oracle action-audit guard and legacy value-filter anchor. The table should show the guard as the primary result and retain the 5-seed value-filter comparison as historical descriptive context.
