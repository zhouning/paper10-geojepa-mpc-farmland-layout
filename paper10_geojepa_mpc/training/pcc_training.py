...
def train_pcc_ensemble(
    *,
    ...
    by_seed = _load_artifact_arrays(manifest_path, manifest)
    unique_ids = np.asarray(sorted(by_seed), dtype=np.int64)
    all_data = _concatenate_datasets(by_seed, unique_ids)
    objective_scaling = compute_robust_objective_scaling(
        all_data["target"],
        all_data["reference_target"],
    )
    ...
    for member_index in range(int(ensemble_size)):
        ...
        arrays = _concatenate_datasets(by_seed, bootstrap_ids)
        model = PCCGeoJEPAMember(
            block_feature_dim=arrays["block"].shape[-1],
            k_global=arrays["global"].shape[-1],
        ...
        n_samples = tensors["actions"].shape[0]
        totals = {
            "loss": 0.0,
        ...
        for _ in range(int(epochs)):
            order = torch.randperm(n_samples, device=device_obj)
            for start in range(0, n_samples, int(batch_size)):
                indexes = order[start : start + int(batch_size)]
                candidate = model(
                    tensors["block"][indexes],
                    tensors["neighbour"][indexes],
                    tensors["global"][indexes],
                    tensors["actions"][indexes],
                )
                reference = model(
                    tensors["block"][indexes],
                    tensors["neighbour"][indexes],
                    tensors["global"][indexes],
                    tensors["reference_actions"][indexes],
                )
                candidate_target = (tensors["target"][indexes] - center) / scale
                reference_target = (
                    tensors["reference_target"][indexes] - center
                ) / scale
                transition_huber = 0.5 * (
                    F.smooth_l1_loss(
                        candidate.next_block,
                        tensors["candidate_next_bf"][indexes],
                    )
                    + F.smooth_l1_loss(
                        candidate.next_global,
                        tensors["candidate_next_gf"][indexes],
                    )
                    + F.smooth_l1_loss(
                        reference.next_block,
                        tensors["reference_next_bf"][indexes],
                    )
                    + F.smooth_l1_loss(
                        reference.next_global,
                        tensors["reference_next_gf"][indexes],
                    )
                )
                objective_nll = 0.5 * (
                ...
                totals["transition_huber"] += float(
                    transition_huber.detach().cpu()
                )
                totals["objective_nll"] += float(objective_nll.detach().cpu())
                totals["rank_loss"] += float(rank_loss.detach().cpu())
                totals["executable_bce"] += float(executable_bce.detach().cpu())
                updates += 1
