...
def test_torch_model_mpc_adapter_can_emit_value_scores_as_rewards():
    ...
    assert np.array_equal(aux["value"], np.array([8.0, 3.0], dtype=np.float32))
...
def test_torch_model_mpc_adapter_can_emit_blended_scores_as_rewards():
    ...
    assert aux["score_mode"] == "blend"
    assert aux["value_weight"] == 0.25
    assert np.array_equal(aux["value"], np.array([8.0, 3.0], dtype=np.float32))


def test_torch_model_mpc_adapter_exposes_value_aux_in_reward_mode():
    adapter = TorchModelMPCAdapter(
        ActionIdRewardValueModel(),
        n_blocks=4,
        device="cpu",
    ...
    assert np.array_equal(rewards, np.array([2.0, 7.0], dtype=np.float32))
    assert np.array_equal(aux["value"], np.array([8.0, 3.0], dtype=np.float32))
