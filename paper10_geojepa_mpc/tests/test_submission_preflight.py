...
def test_submission_preflight_minimal_fixture_reports_missing_figure_table_caption_claim_packet(tmp_path):
    fixture = copy_minimal_preflight_fixture(tmp_path)
    (fixture / PAPER10_FIGURE_TABLE_CAPTION_CLAIM_PACKET_MD).unlink()

    result, payload = run_submission_preflight_json(fixture)

    assert result.returncode == 1
    assert payload["ok"] is False
    assert "paper10_figure_table_caption_claim_packet_current" in payload["failed_checks"]
...
def test_submission_readiness_boundary_rejects_ready_to_submit_claim(tmp_path):
    fixture = copy_minimal_preflight_fixture(tmp_path)
    boundary = fixture / PAPER10_SUBMISSION_READINESS_BOUNDARY
    boundary.write_text(
        boundary.read_text(encoding="utf-8")
...
def test_submission_preflight_minimal_fixture_reports_missing_real_env_longhorizon_confirmatory_audit(tmp_path):
    fixture = copy_minimal_preflight_fixture(tmp_path)
    (fixture / PAPER10_REAL_ENV_LONGHORIZON_CONFIRMATORY_AUDIT_MD).unlink()

    result, payload = run_submission_preflight_json(fixture)

    assert result.returncode == 1
    assert payload["ok"] is False
    assert "paper10_real_env_longhorizon_confirmatory_audit_current" in payload["failed_checks"]
